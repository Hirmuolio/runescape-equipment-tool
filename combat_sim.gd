...
func calc_p_max_hit( player : player, target_mon : monster ):
	...
	# Melee
	var eff_str = floor( (floor( player.strength * player.prayer_str ) + player.style_str_bonus + 8) * player.other_str_bonus )
	
	p_max_hit = 0.5 + eff_str * ( player.str_bonus + 64 ) / 640
...
func calc_p_hit_chance( player : player, target_mon : monster ):
	...
	var atk_roll = floor( eff_atk * ( player.atk_bonus  + 64 ) ) # * gear_bonus
